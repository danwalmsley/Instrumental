...
internal sealed class UdpMetricsListener : IAsyncDisposable
{
    ...
    {
        if (MonitoringEnvelopeSerializer.TryDeserializeBatch(buffer, _preferredEncoding, out var envelopes) ||
            MonitoringEnvelopeSerializer.TryDeserializeBatch(buffer, out envelopes))
        {
    ...
    private void HandleEnvelope(MonitoringEnvelope envelope)
    {
